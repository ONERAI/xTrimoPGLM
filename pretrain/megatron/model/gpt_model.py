...
class GPTModel(MegatronModule):
    ...
    def forward(self, input_ids, position_ids, attention_mask, labels=None,
                tokentype_ids=None, layer_past=None, get_key_value=False, get_embs = False,
        ...
        if self.post_process:
            ret_logits = post_language_model_processing(
                lm_output, labels,
...
class GPTModelPipe(PipelineModule,MegatronModule):
    ...
    def __init__(
        ...
        def _logits_helper(embedding, lm_output):
            ...
            ret_logits = parallel_lm_logits(
                lm_output,
                embedding.word_embeddings_weight,
                self.parallel_output)
            # if mpu.is_pipeline_last_stage() and (torch.distributed.get_rank() % 4 == 0):
            # print(f"rank: {torch.distributed.get_rank() }  embeddings: {ret_logits[0][10][:3]}", flush=True)
            # breakpoint()
            return ret_logits
