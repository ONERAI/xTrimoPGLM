...
def parse_args(extra_args_provider=None, defaults={},
    ...
    if args.deepspeed and args.use_deepspeed_dist_init:
        deepspeed.init_distributed()
        args.local_rank = int(os.getenv('LOCAL_RANK', '0'))
        print(f"pod_name: {os.getenv('HOSTNAME')}, world_rank: {os.getenv('OMPI_COMM_WORLD_RANK')}")        
    ...
    if args.position_embedding_type == PositionEmbeddingType.absolute or (
            args.position_embedding_type == PositionEmbeddingType.alibi and not args.glm):
...
def _add_distributed_args(parser):
    ...
    group.add_argument('--use-deepspeed-dist-init', action='store_true',
                       help='If set, use deepspeed distribution init, needed for k8s mpi launcher')
    return parser
...
def _add_glm_args(parser):
    ...
    group.add_argument("--bert-prob", type=float, default=0.8)
    group.add_argument("--span-prob", type=float, default=0.1)
    ...
    # --span-len-per-sample
    group.add_argument("--span-len-per-sample", type=int, default=None)
    ...
    group.add_argument('--multitask-data-transform', action='store_true',
                       help='If set, change the multitask ratio dynamically.')
    return parser
