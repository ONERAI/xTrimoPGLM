...
class AnnealingLR(object):
    ...
    def get_lr(self):
        ...
        if (
            ...
            c_lr = self.min_lr * (max_decay_steps - iteration) / (max_decay_steps-self.lr_auto_warmup_steps[0])
            return c_lr
        else:
            return self.min_lr
    ...
    def get_lr_wp(self):
        ...
        args = get_args()
        iteration = args.iteration if hasattr(args, 'iteration') else 0
        #print(f'iteration_debug: {iteration} auto_warmup_steps: {self.lr_auto_warmup_steps}')
        if (
            self.lr_auto_warmup_steps is not None
            ...
            #)
            c_lr = self.min_lr * (iteration - self.lr_auto_warmup_steps[0]) / self.lr_auto_warmup_steps[1]
            #print(f'######iteration_debug: {iteration} auto_warmup_steps: {self.lr_auto_warmup_steps} c_lr: {c_lr}')
            return c_lr
        else:
            return self.min_lr
