...
def build_train_valid_test_datasets(
    ...
    collator = GLMPreprocessor(
        ...
        span_seq_length=args.span_len_per_sample,
    ...
    if data_prefix is not None:
        ...
        for prefix in data_prefix:
            text_datasets.append(BinaryDataset(
                f"{prefix}" if prefix.endswith('.bin') else f"{prefix}.bin",
...
def build_single_mask_matrix(separator, batch_size, seq_length, task_type, memory_length=0):
    dtype = torch.float
    # m = torch.ones(
    ...
    m = torch.ones((batch_size, seq_length, seq_length), dtype=dtype, device=separator.device)
    m = torch.tril(m)
    ...
    for idx, i in enumerate(task_type.view(-1)):
        if not i:
            division = separator[idx]
            # m[idx][division:, division:] = 0
            m[idx, division:, :] = 0
            m[idx, :, division:] = 0
    # breakpoint()
    m = m.unsqueeze(1)
...
def build_single_mask_matrix_samples(separator, seq_length, memory_length=0):
    dtype = torch.float
    # m = torch.ones(
    #     (1, seq_length, seq_length), dtype=dtype, device=separator.device
    # )
...
def build_mask_matrix(separator, batch_size, seq_length, task_type):
    ...
    if separator.dim() == 1:
        ...
        for idx in range(bs):
            single_length = seq_length // torch.count_nonzero(separator[idx]).item()
            for i in range(aggregated_samples):
                # print(f"idx: {idx} i: {i} {single_length} {separator[idx][i]} {not separator[idx][i]}")
                if not separator[idx][i]:
                    continue
                single_mask = build_single_mask_matrix_samples(separator[idx][i], seq_length=single_length)
                m[idx, :, single_length * i: single_length * (i + 1), single_length * i: single_length * (i + 1)] = single_mask
        # breakpoint()
        return m
